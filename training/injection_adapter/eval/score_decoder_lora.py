...
def main():
    ...
    ap.add_argument("--out", default=None, help="override output path (default <outdir>/<tag>.jsonl)")
    ...
    print(f"[{args.tag}] {len(rows)} total, {len(done)} done, {len(todo)} to score "
          f"| base={args.base} adapter={'NONE' if args.no_adapter else args.adapter}", flush=True)
    ...
    for i in range(0, len(todo), B):
        chunk = todo[i:i + B]
        res = injscore.injection_scores(model, tok, [r["text"] for r in chunk], pos_id, neg_id,
                                        max_len=args.max_len, batch_size=B, device=dev)
