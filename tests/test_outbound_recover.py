...
REAL_DECLINES = [
    "Done. Started a clean new project: `/home2/srallaba/projects/project_agent_guardrail_drift` "
    "-- scaffolding is in place.",
    "The local repo is already set up correctly here: "
    "`/home2/srallaba/projects/project_webpage/repos/saikrishnarallabandi.github.io`",
]
...
def test_path_collapses_to_bare_filename_not_a_redaction_marker():
    ...
    r = guard(use_llm=False).recover(
        "The outbound guard lives in /home2/srallaba/projects/jataayu/jataayu/guards/outbound.py "
...
def test_rewrite_that_reintroduces_a_credential_is_withheld():
    ...
    r = g.recover("scaffolding is in /home2/srallaba/projects/foo", surface=GROUP)
...
def test_llm_that_still_leaks_falls_back_to_deterministic_redaction():
    ...
    llm = StubLLM(reply="Sure! It's at /home2/srallaba/projects/still_leaking/x.py")
    g = guard(llm=llm, use_llm=True)
...
def test_residue_is_fed_back_on_retry():
    ...
    class TwoTries(StubLLM):
        def call(self, system_prompt, user_message, max_tokens=1024):
            self.calls.append((system_prompt, user_message))
            if len(self.calls) == 1:
                return "Nearly: /home2/srallaba/projects/foo is done"  # still leaking
