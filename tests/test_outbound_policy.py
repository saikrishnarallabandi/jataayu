...
class TestProtectedNamesFromPolicy:
    ...
    def test_without_the_policy_the_same_name_passes(self, tmp_path):
        ...
        result = jataayu_check_outbound("My daughter Alice loves coding", surface="discord-channel")
...
class TestEveryRevivedKeyChangesBehaviour:
    def test_internal_codenames_block_on_every_surface(self, tmp_path):
        ...
        assert (
            jataayu_check_outbound(text, surface="internal", policy_file=policy, agent="bot")[
                "status"
            ]
            == "BLOCK"
        )
    ...
    def test_concurrent_policy_files_never_leak_each_others_names(self, tmp_path):
        ...
        def worker(name):
            ...
            for _ in range(2500):
                ...
                sent = (
                    result["redacted"] if result["redacted"] is not None else f"{name} shipped it"
                )
                if name in sent:
