...
class TestCheckOutbound:
    ...
    def test_protected_names(self):
        result = jataayu_check_outbound(
            "Alice is doing great at KidStrong this week!",
            surface="discord-channel",
            protected_names=["Alice", "Carol", "Bob"],
        )
        assert result["status"] in ("WARN", "BLOCK")
        assert result["redacted"] is not None
        assert "Alice" not in result["redacted"]

    def test_protected_names_redaction(self):
        result = jataayu_check_outbound(
            "Tell Carol and Bob that dinner is ready.",
            surface="whatsapp",
            protected_names=["Carol", "Bob"],
        )
        assert result["redacted"] is not None
        assert "Carol" not in result["redacted"]
        assert "Bob" not in result["redacted"]
