...
#   nohup bash benchmarks/agentdojo/sweep_hosted.sh > runs/agentdojo/sweep_hosted.log 2>&1 &
set -uo pipefail

REPO="${REPO:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
PY="${PY:-$REPO/benchmarks/.venv/bin/python}"
...
for spec in $MODELS; do
  ...
  for suite in $SUITES; do
    OUT="$REPO/benchmarks/results/agentdojo_${suite}_${tag}_sweep.json"
    ...
    "$PY" benchmarks/agentdojo/run_agentdojo.py \
...
notify "[jataayu] Phase B HOSTED sweep done ($MIN_STATUS): $MODELS x $SUITES. Results: benchmarks/results/agentdojo_<suite>_<tag>_sweep.json."
