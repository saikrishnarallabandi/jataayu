...
integration (`benchmarks/agentdojo/jataayu_defense.py`) exercises only the text detector
...
house eval conventions (`benchmarks/run_injection_bench.py` et al.).
...
attack/legit split. Build `benchmarks/data/effect_boundary_v1.jsonl`, one JSON row per action:
...
- **File:** `benchmarks/run_effect_boundary_bench.py` (standalone, house style — no shared eval lib exists).
- **Imports (reuse as-is):**
