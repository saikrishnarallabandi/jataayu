...
Emits benchmarks/data/composition_v1.jsonl — a balanced set of skill *sets*, each row a
...
def build() -> list[dict]:
    ...
    rows.append(
        {
            ...
            "class": "RISKY",
            "skills": [
                _uniq(rng, "reads_secrets", used),
                _uniq(rng, "reads_secrets", used),
                _uniq(rng, "network_write", used),
            ],
            "expected_verdict": "REVIEW",
    ...
    rows.append(
        {
            ...
            "skills": [
                _uniq(rng, "network_read", used),
                _uniq(rng, "fs_write", used),
                _uniq(rng, "exec", used),
                _uniq(rng, "memory_read", used),
            ],
            "expected_verdict": "REVIEW",
    ...
    for j, (allowed, cap) in enumerate(
        [(["network_read", "fs_read"], "fs_read"), (["fs_read", "network_read"], "network_read")]
    ):
        used = set()
        rows.append(
            {
