...
_INJECTION_BLOCK_RES: list[re.Pattern] = [
    ...
    re.compile(
        r"<\s*(SYSTEM|INST|IMPORTANT|ADMIN)\s*>.*?(?:<\s*/\s*\1\s*>|\Z)", re.IGNORECASE | re.DOTALL
    ),
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith("#"):
                if _MARKDOWN_INJECTION_KEYWORDS.search(stripped):
                    findings.append(
                        f"NLP-HEADING: Injection keyword in markdown heading: {stripped[:80]!r}"
                    )
            ...
            # Check non-code paragraphs for authority claims combined with dangerous actions
                ...
                if _MARKDOWN_INJECTION_KEYWORDS.search(stripped):
                    ...
                    if re.search(
                        r"\b(ai|assistant|agent|claude|gpt|llm|you|model)\b",
                        stripped,
                        re.IGNORECASE,
                    ):
                        findings.append(
                            f"NLP-AUTHORITY: Authority claim with injection directive: {stripped[:80]!r}"
                        )
                        max_score = max(max_score, 0.72)
                    else:
                        findings.append(
                            f"NLP-HIDDEN: Injection directive in paragraph: {stripped[:80]!r}"
                        )
            ...
            # Check list items for injection directives
                ...
                if _MARKDOWN_INJECTION_KEYWORDS.search(item_text):
                    findings.append(
                        f"NLP-LIST: Injection directive in list item: {item_text[:80]!r}"
                    )
