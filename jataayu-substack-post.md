...
config = PrivacyConfig(
    protected_names=["Alice", "Bob", "Alice"],
    use_llm=False  # pattern-only for speed
)
guard = OutboundGuard(config=config)

draft = "Alice's daughter Alice just started at Lincoln Elementary. She's 3 years old."
result = guard.check(draft, surface="discord-channel")

print(result.blocked)      # True
print(result.explanation)  # "Contains protected names: Alice, Alice"
```
...
Jataayu now runs as a plugin in [gateway](https://gateway.dev), exposed as two first-class tools the agent can call:
