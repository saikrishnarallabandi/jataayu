...
address / ZIP) are drawn from ``ai4privacy/pii-masking-200k`` (a bounded,
deterministic first-N sample of the English rows). If that set will not
download, the harness falls back to the curated ``standard_pii`` positives in
``eval/data/outbound_privacy_v1.jsonl``.
...
``python eval/data/gen_outbound_privacy_v1.py``.
...
Usage:
  python eval/run_outbound_privacy_bench.py
  python eval/run_outbound_privacy_bench.py --hf-limit 400 --surface public --json
  python eval/run_outbound_privacy_bench.py --no-hf     # curated corpus only
"""
import argparse
import json
    ...
    try:
        for row in ds:
            ...
            for s in row.get("privacy_mask", []):
                cat, _in = cat_for(s["label"])
                spans.append({"value": s["value"], "label": s["label"], "category": cat})
            rows.append({
...
def main():
    ...
    ap.add_argument("--hf-limit", type=int, default=400,
                    help="max English ai4privacy rows to sample (deterministic prefix)")
    ...
    if args.no_hf:
        ...
        if hf_rows:
            std, corpus_source = hf_rows, "ai4privacy/pii-masking-200k (public)"
        else:
    ...
    for rec, r in pos:
        ...
        for cat in cats_in_msg:
            ...
            if is_flag:
                det_msgs[cat][0] += 1
        # any in-scope span present?
        if any(cat_for(sp["label"])[1] for sp in rec["spans"]):
            in_scope_flags[1] += 1
            if is_flag:
                in_scope_flags[0] += 1
