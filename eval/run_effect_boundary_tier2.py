...
This runner is the disclose-then-fix harness (spec §5). Run it UNCHANGED twice:
  (i)  as-shipped:  python eval/run_effect_boundary_tier2.py --out eval/results/effect_boundary_tier2_asshipped.json
  (ii) after extending jataayu/core/taint.py with the suite tools -> re-run:
       python eval/run_effect_boundary_tier2.py --out eval/results/effect_boundary_tier2_fixed.json
