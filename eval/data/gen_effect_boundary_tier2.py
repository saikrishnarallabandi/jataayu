...
Emits eval/data/effect_boundary_tier2.jsonl — one labeled tool call per row, drawn from the
REAL tool names of AgentDojo's four suites (workspace, banking, travel, slack) plus a small
"runtime" band of agent tools (shell/code-eval/secret-read/memory-write) so the corpus spans
...
Design (see eval/EFFECT_BOUNDARY_BENCHMARK_SPEC.md §3 Tier 2, §5 disclose-then-fix):
