# Jataayu gateway plugin (enforced)

Wires Jataayu into the gateway agent gateway (the "Judith pipeline") as an **enforced**
security layer — not an advisory tool the model can skip.

Source of truth lives here in the Jataayu repo. Deployed copies at
`~/.gateway/plugins/jataayu/` should be regenerated from this directory via `./deploy.sh`.
...
- **Fail-open**: the handler catches all errors and returns an explicit `{outcome:"pass"}`, and the
  Jataayu check times out at 6 s — well under gateway's 15 s `before_agent_run` fail-closed timeout —
  so a guard fault can never block message delivery.
- `blockOnInboundHigh` (default true) toggles enforce vs warn/log-only.
- Observe it firing: `journalctl --user -u gateway-gateway.service -f | grep jataayu-gate`
