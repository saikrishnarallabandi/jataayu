#!/usr/bin/env bash
# Deploy the Jataayu OpenClaw plugin -- index.js, the manifest, tests, package metadata, AND modules/.
...
# under ~/.openclaw/plugins/ and were required across directories. They existed NOWHERE in this
...
echo "  journalctl --user -u openclaw-gateway -n 300 | grep -E 'jataayu|group-guard|group-capture|dm-guard'"
