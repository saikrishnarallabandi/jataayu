 ...
 * (~/.openclaw/extensions/whatsapp/dist/monitor-*.js), hooking
 * `skipGroupMessageAndStoreHistory` to append every ambient group message to
 * memory/group-capture/YYYY-MM-DD.jsonl.
 *
 * That patch is deleted by any openclaw update — the extension is re-unpacked from its
 * clawhub tarball. It survived the 2026.5.27 bump only by luck, and the 2026.6.11 upgrade
 ...
 * remain, and an openclaw update can no longer silently delete our behaviour.
 *
 ...
 * absent is written as null rather than guessed. Set OPENCLAW_GROUP_CAPTURE=debug to log the
...
const DIR = path.join(
  process.env.HOME || "/home/srallaba",
  ".openclaw", "workspace", "memory", "group-capture",
);
...
function activate(api) {
  const debug = (process.env.OPENCLAW_GROUP_CAPTURE || "").toLowerCase() === "debug";
