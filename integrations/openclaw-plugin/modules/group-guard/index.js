 ...
 * (~/.openclaw/extensions/whatsapp/dist/access-control-*.js). That file is re-unpacked
 * from its clawhub tarball whenever the extension is provisioned, so an OpenClaw update
 ...
 * "Re-apply after openclaw update." It survived the 2026.5.27 bump only by luck.
 *
 ...
 *
 * MODE: set OPENCLAW_GROUP_GUARD=observe to log decisions WITHOUT blocking. Used to
 * prove the hook fires correctly before handing it the security role. `enforce` blocks.
 */
const fs = require("node:fs");
const path = require("node:path");

const PIN_PATH = path.join(process.env.HOME || "/home/srallaba", ".openclaw", "group-pin.json");
const CACHE_MS = 5000;
...
function activate(api) {
  const mode = (process.env.OPENCLAW_GROUP_GUARD || "enforce").toLowerCase();
