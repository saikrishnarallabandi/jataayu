/**
 * dm-guard unit tests — run with:  node /home/srallaba/.openclaw/plugins/dm-guard/test.js
 *
...
(async () => {
...

  {
    ...
    const r = await g.send(SAI_DM,
      ...
      "/home/srallaba/.openclaw/plugins/jataayu/index.js. Tests 242 green.");
    check("DM with absolute paths -> sent, rewritten to ~ (receipt survives)", () => {
      ...
      assert(/~\/\.openclaw\/plugins\/jataayu\/index\.js/.test(r.content), r.content);
  ...
  {
    // build a throwaway openclaw.json with a systemPrompt, and echo a chunk of it back
